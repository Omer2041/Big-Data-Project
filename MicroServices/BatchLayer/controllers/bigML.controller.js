...
const connection = new bigml.BigML(process.env.MY_USERNAME, process.env.MY_API_KEY);
...
const buildModel = async (req, res) => {
  /* Prepare the Dataset */
  const orders = await getOrders(/* req.body ? */); // Get all orders from MongoDB
  ...
  source.createAndWait(DATASET_PATH, function(error, sourceInfo) {
    if (error) { console.log('Error creating source'); throw error; }
    ...
    dataset.create(sourceInfo, function(error, datasetInfo) {
      if (error) { console.log('Error creating dataset'); throw error; }
      ...
      association.create(datasetInfo, { search_strategy: 'support' }, function (error, associationInfo) {
        if (error) { console.log('Error creating association'); throw error; }

        /* Association created successfully */
        console.log(`Association created: ${associationInfo.resource}`);
        getAssociationRules(associationInfo.resource);
      });
    });
  });
}

const getAssociationRules = (associationId) => {

  const model = new bigml.Model(connection);
  model.get(associationId, true, (err, modelInfo) => {
    if (err) { console.log('Error getting model'); throw err; }
    console.log(modelInfo);
    const rules = modelInfo.object.associations.rules;
    if (!rules) { console.error(`No associations found (${associationId})`); return; }
    const items = modelInfo.object.associations.items;
    if (!items) { console.error(`No items found (${associationId})`); return; }

    const sets = extractRules(rules, items);
    console.log(sets);
    fs.writeFileSync(RESULTS_PATH, JSON.stringify(sets, null, '\t'), { encoding: 'utf-8', flag: 'w' });
    console.log('Finished building model, results were written to ' + RESULTS_PATH);
  });
}
...
const extractRules = (rules, items) => {
  ...
  for (let i = 0; i < rules.length; ++i) {
    ...
    for (let item = 0; item < left.length; ++item) {
      set.push(items[left[item]].name);
    }
    set.push("->");
    for (let item = 0; item < right.length; ++item) {
      set.push(items[right[item]].name);
    }
